...
public class AccountVerificationController {
    ...
    @Operation(summary = "Sign Account Verification Request (acmt.023)",
            description = "Accepts a 3-field JSON request from Core Banking (receiverBic, accountIdentifier, identifierType: ACCT/EWLT/MSIS/IBAN), injects bank's BIC from application.yaml, builds ISO 20022 acmt.023.001.03 XML, and digitally signs it using private.pem & certificate.pem.")
    @ApiResponses({
            @ApiResponse(responseCode = "201", description = "ISO 20022 acmt.023 message created and digitally signed successfully"),
    ...
    @PostMapping("/sign")
    public ResponseEntity<SignedAcmt023Response> signAcmt023Request(
            @RequestBody @Valid AccountVerificationRequestJson request
    ) {
        SignedAcmt023Response response = accountVerificationService.createAndSignAcmt023(request);
