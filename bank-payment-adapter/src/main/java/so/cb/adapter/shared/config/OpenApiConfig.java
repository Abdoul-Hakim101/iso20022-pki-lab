...
public class OpenApiConfig {
    ...
    public OpenAPI customOpenAPI() {
        return new OpenAPI()
                .info(new Info()
                        .title("Bank Payment Adapter API")
                        .version("1.0.0")
                        .description("""
                                API documentation for the Bank Payment Adapter Service.
                                ...
                                • Outgoing Workflow (Core Banking -> Adapter -> Network): Validates JSON payment requests, maps JSON to ISO 20022 XML (e.g. acmt.023), digitally signs XML using bank's private key (private.pem), and transmits to network.
                                • Incoming Workflow (Network -> Adapter -> Core Banking): Validates Central Bank Root CA trust anchor (chain.pem), verifies W3C XML Digital Signatures, validates ISO 20022 XML payload, transforms XML to bank JSON format, and forwards requests to Core Banking.
